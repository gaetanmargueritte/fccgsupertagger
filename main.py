    ...
    with open(OUTPUTS_FOLDER + "inputs.conll", "w") as f:
        for s in sentences:
            ...
            for doc in sentences[s]:
                f.write(
                    f"{i}\t{doc.text}\t{doc._.lefff_lemma}\t{DICT_COARSE_POSTAG[doc._.melt_tagger]}\t{doc._.melt_tagger}\t_\n"
                )
                i += 1
            f.write("\n")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        print("**** Done. ****")
        predictions = model(
            batch_words, batch_lemmas, batch_postags, batch_deprels, batch_masks
        )
        end = time.time()
