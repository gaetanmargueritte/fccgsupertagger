...
from camembert_model import CamemBERTTagger
...
def decode_outputs(
    model: CamemBERTTagger, words: List[List[int]], word_ids: List[List[int]], outputs: List[List[int]], id2tags: Dict[int, str]
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        tokenized_data = model.tokenize_inputs(data, max_sequence_length=model_data["max_sequence_length"])
        inputs_len = len(tokenized_data)
        print("**** Done. ****")
        # clumsily avoid VAE hidden state management outside of CamemBERTTagger. 
        # Future version will (as it should) manage instead hidden states to adat to input size.
        while len(tokenized_data) < model_data["batch_size"]:
            tokenized_data.append(({"input_ids": [5, 6], "attention_mask": [1, 1], "words_id": [None, None]}))
        ...
        sentences, tags = decode_outputs(model, words_batch, words_id, predictions, id2tags)
        print("----------------")
        for s, t in zip(sentences,tags):
            for word, tag in zip(s.split(), t):
                print(f"{word:>12}\t\t{tag:<1}")
            print("----------------")
