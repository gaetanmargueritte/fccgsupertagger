...
class Tagger(nn.Module):
    def __init__(
        ...
        # lemma
        self._lemma_embed = nn.Embedding(
            self._lemma_embed_size, self._max_embedding_dim
        )
        torch.nn.init.xavier_uniform_(self._lemma_embed.weight)
        # postag
        self._postag_embed = nn.Embedding(
            self._postag_embed_size, self._postag_embed_size
        )
        torch.nn.init.xavier_uniform_(self._postag_embed.weight)
        # deprel
        self._deprel_embed = nn.Embedding(
            self._deprel_embed_size, self._deprel_embed_size
        )
    ...
    def nll(
        ...
        predictions = self.get_sentence_prediction_emission(
            sentence, lemma, postag, deprel
        )
    ...
    def _merge_feature_correlations(
        self, embeds_list: List[Tensor], sentence_length: int
    ) -> Tuple[Tensor]:
    ...
    def get_sentence_prediction_emission(
        self, sentence: Tensor, lemma: Tensor, postag: Tensor, deprel: Tensor
    ) -> Tensor:
        # embedding inputs
