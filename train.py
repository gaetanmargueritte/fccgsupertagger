...
from utils import load_dataset, load_words_embedding_file, build_vocab, enhance_vocabulary, pad_sequence, shuffle_and_split, get_data_from_dataset, PAD_TAG



def evaluate(model: Tagger, data: List[Dict[str, List[str]]], best_acc: float, log_result: bool = False):
    def get_batch(batch_index: int, dataset: List):
        ...
        words_batch_text, words_batch, lemmas_batch, postags_batch, deprels_batch, ccgs_batch = [], [], [], [], [], []
        masks_batch = []
        for sentence in batch:
            masks_batch.append(pad_sequence([1 for _ in range(len(sentence["sentence_text"]))], max_sequence_length, 0))
            words_batch_text.append(sentence["sentence_text"])
            words_batch.append(pad_sequence(sentence["words_id"], max_sequence_length, words2id[PAD_TAG]))
            lemmas_batch.append(pad_sequence(sentence["lemmas_id"], max_sequence_length, lemmas2id[PAD_TAG]))
            postags_batch.append(pad_sequence(sentence["postags_id"], max_sequence_length, postags2id[PAD_TAG]))
            deprels_batch.append(pad_sequence(sentence["deprels_id"], max_sequence_length, deprels2id[PAD_TAG]))
            ccgs_batch.append(pad_sequence(sentence['ccgs_id'], max_sequence_length, ccgs2id[PAD_TAG]))
        ...
        return (words_batch_text, words_batch, lemmas_batch, postags_batch, deprels_batch, ccgs_batch, masks_batch)
    ...
    with torch.no_grad():
        for _ in range(number_batch_per_epoch):
            words_text, words, lemmas, postags, deprels, ccgs, masks = get_batch(batch_index, data)
            batch_index += 1
            predictions = model(words, lemmas, postags, deprels, masks)
            for i in range(len(predictions)):
                for (word, predicted_tag, true_tag) in zip(words_text[i], predictions[i], ccgs[i]):
                    ...
                    if predicted_tag == true_tag:
                        ...
                        errors[true_tag] = errors[true_tag] + 1 if true_tag in errors else 1
                    total += 1
                prediction.append('')
...
def train_model():
    ...
    lr = 0.015
    optimizer = torch.optim.RMSprop(model.parameters(), lr=lr, weight_decay=1e-4, momentum=0.9)
    lr_reduce = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", patience=1, factor=0.2, verbose=True, threshold_mode='abs', threshold=1e-3)
    ...
    for i in range(epochs):
        ...
        model.train(False)
        accuracy_train, best_accuracy_train = evaluate(model, data_train, best_accuracy_train)
        previous_best = best_accuracy_valid
        accuracy_valid, best_accuracy_valid = evaluate(model, data_valid, best_accuracy_valid)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", default="model.pt", type=str, help="model file")
    parser.add_argument("-ep", "--epochs", default=60, type=int, help="Number of training epochs (default: 60)")
    parser.add_argument("-em", "--embed", default="data/frWac_non_lem_no_postag_no_phrase_200_skip_cut100.bin", type=str, help="binary word embedding file from word2vec (default: data/frWac_non_lem_no_postag_no_phrase_200_skip_cut100.bin)")
    parser.add_argument("-d", "--dataset", default="data/ccgresult28.txt", type=str, help="dataset file of FrenchCCGBank in txt format (default: data/ccgresult28.txt)")
    parser.add_argument("-vr", "--validation-ratio", default=0.2, type=float, help="validation/training ratio for training (default: 0.2)")
    parser.add_argument("-tr", "--test-ratio", default=0.2, type=float, help="test/dataset ratio for training (default: 0.2)")
    parser.add_argument("-s", "--seed", default=0, type=int, help="random seed (default: 0)")
    parser.add_argument("-we", "--word-embedding", default=200, type=int, help="word embedding size (default: 200)")
    parser.add_argument("-b", "--batch-size", default=16, type=int, help="batch size (default: 16)")
    ...
    word_embeds = np.random.uniform(-np.sqrt(0.06), np.sqrt(0.06), (len(words2id), word_embedding_dim))
    ...
    dataset_train, dataset_valid, dataset_test = shuffle_and_split(dataset, vratio, tratio, seed)
    print(f"Lengths of training / validation / test datasets in number of sentences: {len(dataset_train)} / {len(dataset_valid)} / {len(dataset_test)}")

    data_train = get_data_from_dataset(dataset_train, words2id, lemmas2id, postags2id, deprels2id, ccgs2id)
    data_valid = get_data_from_dataset(dataset_valid, words2id, lemmas2id, postags2id, deprels2id, ccgs2id)
    data_test = get_data_from_dataset(dataset_test, words2id, lemmas2id, postags2id, deprels2id, ccgs2id)
